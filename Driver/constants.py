...
# Tier 1 invariant "Cilium 1.19.3 agent healthy" (brief Section 11) pins an
# exact version, not just "Cilium is running" — Infra/files(1)/setup.sh
# installs this exact chart version. Single source of truth; driver.py's
# check_cilium_healthy() checks the running agent's image tag against this,
# not just pod Running/Ready (a version drift previously wasn't caught).
